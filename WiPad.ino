...
#  else
      if ( (buf[2] & 0xf0) == 0xf0 && buf[5] == 0x7f && buf[6] == 0x7f ) {
        ...
        // player 1
        DBG("PS2Dual ", false);
        isUp =    ( buf[4] == 0x00 );     if ( isUp ) DBG("UP");
        isRight = ( buf[3] == 0xff );     if ( isRight ) DBG("RT");
        isDown =  ( buf[4] == 0xff );     if ( isDown ) DBG("DN");
        isLeft =  ( buf[3] == 0x00 );     if ( isLeft ) DBG("LT");
        isA = ( buf[1] & 0x04 );   if ( isA ) DBG("A");
        isB = ( buf[1] & 0x02 );   if ( isB ) DBG("B");
        isC = ( buf[1] & 0x80 || buf[1] & 0x10 );   if ( isC ) DBG("C");
        isX = ( buf[1] & 0x08 );   if ( isX ) DBG("X");
        isY = ( buf[1] & 0x01 );   if ( isY ) DBG("Y");
        isZ = ( buf[1] & 0x40 || buf[1] & 0x20 );   if ( isZ ) DBG("Z");
        isStart = ( buf[2] & 0x01 );    if ( isStart ) DBG("Start");
        isSelect = ( buf[2] & 0x02 );   if ( isSelect ) DBG("Select");
      }
      else if ( buf[0] == 0x01 && buf[1] == 0x7F ) {
        ...
        isDown =  ( buf[4] == 0xff );     if ( isDown ) DBG("DN");
        isLeft =  ( buf[3] == 0x00 );     if ( isLeft ) DBG("LT");
        isA = ( buf[5] & 0x40 );   if ( isA ) DBG("A");
        isB = ( buf[5] & 0x20 );   if ( isB ) DBG("B");
        isC = ( buf[6] & 0x08 || buf[6] & 0x01 );   if ( isC ) DBG("C");
        isX = ( buf[5] & 0x80 );   if ( isX ) DBG("X");
        isY = ( buf[5] & 0x10 );   if ( isY ) DBG("Y");
        isZ = ( buf[6] & 0x04 || buf[6] & 0x02 );   if ( isZ ) DBG("Z");
        ...
        DBG("8BITDO", false);
        isUp =    ( buf[1] == 0x00 );     if ( isUp ) DBG("UP");
        isRight = ( buf[0] == 0xff );     if ( isRight ) DBG("RT");
        isDown =  ( buf[1] == 0xff );     if ( isDown ) DBG("DN");
        isLeft =  ( buf[0] == 0x00 );     if ( isLeft ) DBG("LT");
        isA = ( buf[5] & 0x04 );   if ( isC ) DBG("A");
        isB = ( buf[4] & 0x20 );   if ( isB ) DBG("B");
        isC = ( buf[4] & 0x10 );   if ( isA ) DBG("C");
        isX = ( buf[4] & 0x80 );   if ( isX ) DBG("X");
        isY = ( buf[5] & 0x01 );   if ( isY ) DBG("Y");
        isZ = ( buf[5] & 0x08 );   if ( isZ ) DBG("Z");
        isStart = ( buf[5] & 0x80 );    if ( isStart ) DBG("Start");
        isSelect = ( buf[5] & 0x40 );   if ( isSelect ) DBG("Select");
      }
